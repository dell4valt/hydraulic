...
def insert_summary_QV_tables(stvors, out_filename):
    ...
    doc.add_paragraph('Таблица — Расчётные уровни {}'.format(stvor.strings['type']), style='Т-название')
    lev_table = doc.add_table(2, cols, style='Table Grid')

    doc.add_paragraph('Таблица — Расчётные скорости {}'.format(stvor.strings['type']), style='Т-название')
    ...
    lev_table.cell(0,4).merge(lev_table.cell(0, len(param_levels[0]) - 1)).text = 'Уровни воды (м БС), обеспеченностью Р%'
    ...
    spd_table.cell(0,4).merge(spd_table.cell(0, len(param_levels[0]) - 1)).text = 'Скорости воды (м/с), обеспеченностью Р%'
    ...
    for stvor in stvors:
        ...
        table_style(lev_table, 'Т-таблица', width=[0.85, 6, 1.25, 1.25, 1.25,])
        table_style(spd_table, 'Т-таблица', width=[0.85, 6, 1.25, 1.25, 1.25,])
